    ...
    def __str__(self):
        if self._sold_products:
            ...
            for i, (product, info) in enumerate(self._sold_products.items(), 1):
                ...
                logger.debug(f'Название проданного товара: {product}')
                logger.debug(f'Количество проданного товара: {info.quantity}')
    ...
    def sell_product(self):
        ...
        if product in self._products:
            ...
            if quantity == self._products[product].quantity:
                logger.warning(f'Сделать заказ продуктов со склада, если {quantity} < 3')
                ...
                logger.error(f'Ожидали, что в магазине достаточно продуков {quantity}. По факту {quantity} >  {self._products[product].quantity}')
                print(f'\n{Back.GREEN}{Fore.BLACK}'
                      f'Товар {product} продан в количестве {quantity}\n'
                      f'Общая стоимость: {total_price} BYN\n'
            ...
            logger.warning(f"Продукт: {product} отсутствует на складе")
            print(f'\n{Back.RED}{Fore.BLACK}'
